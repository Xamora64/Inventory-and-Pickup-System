...
local wait_new_key = false
local new_key = 0

hook.Add( "KeyPress", "keypress_change_key", function( ply, key )
    print(wait_new_key)
	if ( wait_new_key ) then
		new_key = key
        wait_new_key = false
	end
end )
...
hook.Add("PlayerButtonDown", "PlayerButtonDownOpenInventory", function(ply, key)
    if(key == key_open) then
...
hook.Add("PlayerButtonDown", "PlayerButtonDownTake", function(ply, key)
    if(key == key_take) then
        if ply != LocalPlayer() then return end
